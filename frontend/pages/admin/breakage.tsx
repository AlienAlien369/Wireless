    ...
    <AdminLayout title="Breakage Tracking">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div className="card">
            <div className="text-3xl font-bold text-red-600">{breakages.length}</div>
            <div className="text-sm text-gray-600">Total Breakages</div>
          </div>
          <div className="card">
            <div className="text-3xl font-bold text-orange-600">{breakages.length > 0 ? Math.round(breakages.length / (visits.length || 1)) : 0}</div>
            <div className="text-sm text-gray-600">Per Visit Avg</div>
          </div>
          <div className="card col-span-2 md:col-span-1">
            <div className="text-3xl font-bold text-yellow-600">{visits.filter(v => v.isActive).length}</div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <select value={filterVisit} onChange={e => setFilterVisit(e.target.value)} className="input flex-1 md:w-56">
              ...
              <div key={b.id} className="card border-l-4 border-red-500 p-4">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <div className="font-bold text-gray-800">{b.itemNumber}</div>
                    <div className="text-xs text-gray-500 mt-1">{b.visitName}</div>
                  </div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="bg-red-50 rounded p-2 border-l-2 border-red-300">
                    ...
                    <p className="text-red-600 text-xs mt-1">{b.breakageReason}</p>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-gray-600">
                    ...
                    <span className="text-xs">{new Date(b.reportedAt).toLocaleDateString('en-IN')}</span>
                  </div>
                  {b.remarks && (
              ...
              <thead className="bg-gradient-to-r from-red-50 to-red-100 border-b border-red-200">
                <tr>
                  {['Sr.', 'Visit', 'Item#', 'Reason', 'Reported By', 'Date', 'Actions'].map(h => (
                    <th key={h} className="text-left px-4 py-3 font-semibold text-red-700">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {breakages.length === 0 ? (
                  ...
                  breakages.map((b, i) => (
                    <tr key={b.id} className="border-b border-gray-100 hover:bg-red-50 transition-colors">
                      <td className="px-4 py-3 text-gray-500 font-medium">{i + 1}</td>
                      <td className="px-4 py-3 text-xs text-gray-600">{b.visitName}</td>
                      <td className="px-4 py-3 font-bold text-red-700">{b.itemNumber}</td>
                      <td className="px-4 py-3 text-gray-700 max-w-xs">{b.breakageReason}</td>
                      <td className="px-4 py-3 text-gray-700">{b.reportedBy}</td>
                      <td className="px-4 py-3 text-gray-600">{new Date(b.reportedAt).toLocaleDateString('en-IN')}</td>
